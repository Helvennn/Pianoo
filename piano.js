...
const desc = document.getElementById('oct');

x.classList.add('octaved');

body.addEventListener('keypress', function(){
    if(event.key == 'z'){
        z.classList.add('octaved');
        desc.innerHTML = "III";
        if(x.classList.contains('octaved')){
            x.classList.remove('octaved');
        } else if(c.classList.contains('octaved')){
            c.classList.remove('octaved');
        }
    } else if(event.key == 'x'){
        x.classList.add('octaved');
        desc.innerHTML = "IV";
        if(z.classList.contains('octaved')){
            z.classList.remove('octaved');
        } else if(c.classList.contains('octaved')){
            c.classList.remove('octaved');
        }
    } else if(event.key == 'c'){
        c.classList.add('octaved');
        desc.innerHTML = "V";
        if(x.classList.contains('octaved')){
            x.classList.remove('octaved');
        } else if(z.classList.contains('octaved')){
            z.classList.remove('octaved');
        }
    }
})
...
body.addEventListener('keydown', function(){
    ...
    if(key == "1"){
        if(z.classList.contains('octaved')){
            C3.load();
            C3.play();
        } else if(x.classList.contains('octaved')){
            C4.load();
            C4.play();
        } else if(c.classList.contains('octaved')){
            C5.load();
